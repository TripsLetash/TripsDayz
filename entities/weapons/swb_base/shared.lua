...
if CLIENT then
	...
	SWEP.SelectFont = "SWB_SelectIcons"
...
function SWEP:Holster()
	if (self.ReloadDelay and self.ReloadDelay > CurTime()) then
		return false
	end
	...
	if self.SuppressTime and CurTime() < self.SuppressTime then
...
function SWEP:EmptyClip()
	...
	if (self.ReloadDelay and self.ReloadDelay > CT) or (CT < self.ReloadWait) or self.dt.State == SWB_ACTION or self.ShotgunReloadState != 0 then
		return
	...
	if SERVER then
		self.Owner:GiveItem(self.Primary.AmmoItem, self:Clip1())
		self:SetClip1(0)
	end
...
function SWEP:Reload()
	...
	if self.ShotgunReload then
		...
		if !self.ShotgunReload then
            if SERVER then
				if self.NextAmmoCheck > CT then return end
				self.NextAmmoCheck = CT + 1
				
				if self:Clip1() >= self.Primary.ClipSize then return end
				
				local ammocount = self.Primary.ClipSize - self:Clip1()

				if ammocount > self.Owner.InvTable[self.Primary.AmmoItem] then
					ammocount = self.Owner.InvTable[self.Primary.AmmoItem]
				end
				
				self:SetClip1( self:Clip1() + ammocount )
				self.Owner:TakeItem( self.Primary.AmmoItem, ammocount )
            end
		end
	end
	--[[self:SendWeaponAnim(ACT_VM_RELOAD)
	...
	if IFTP then
		if self.ShotgunReloadState == 1 then
			...
			if CT > self.ReloadDelay then
				...
				if SERVER then
					self:SetClip1(mag + 1)
					self.Owner:TakeItem(self.Primary.AmmoItem, 1)
				end
