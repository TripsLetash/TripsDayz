...
concommand.Add( "slotm_spin", function( ply, cmd, args )
	local bet = tonumber(args[1]) or 5
	if bet < 5 then 
		bet = 5
	end
	
	if !ply:HasItemAmount( "item_chips", bet) then 
		ply:ChatPrint("[Casino] You don't have enough chips!")
		return 
	end
	...
	if IsValid(ent) && !ent.SlotsSpinning && !ent.Jackpot then
		...
		ply:TakeItem( "item_chips", math.Round( bet ) )
...
function ENT:SendWinnings( ply, amount, bJackpot )
	...
	if bJackpot then
		for k, v in pairs(player.GetAll()) do 
			v:PrintMessage( HUD_PRINTTALK, "[Casino] " .. ply:Name() .. " WON THE JACKPOT! A total of " .. amount .. " chips!" )
		end
		...
		ply:ChatPrint("[Casino] You won " .. amount .. " chips!" )
	end
	...
	ply:GiveItem( "item_chips", math.Round( amount ) )
